...
非常好问题！下面我会清晰地解释 **logits 是什么**，它与 **概率（probability）**、**softmax**、**交叉熵（cross-entropy）** 等概念之间的关系。

---

## ✅ 一句话定义：

> **Logits** 是神经网络在分类任务中输出的 **未经过 softmax 激活的原始分数向量**。

---

## 🔢 举例说明：

假设一个模型要对一张图像进行 **5 类分类**，网络最后一层（通常是全连接层）输出如下：

```python
logits = [2.3, 0.7, -1.2, 3.1, 0.0]
```
...
* 不保证为正；
* 不保证和为 1；
* 可以看作是每个类别的“原始打分”或“非概率对数分数”。

---

## 🔄 如何变成概率？

用 **softmax** 操作，将 logits 转换为概率分布：
...
## 📊 为什么不直接输出概率，而输出 logits？

### ➤ 数学原因：

softmax + log = log-softmax，是 logsumexp 技术的一部分，**更稳定更高效**。

### ➤ 实践原因：

在训练中，PyTorch 通常不让你手动 softmax，而是使用 `CrossEntropyLoss()`，它内部自动将 logits 变成 log-softmax 进行交叉熵计算。
...
| 概念         | 形式                        | 是否归一化  | 是否可直接比较 |
| ---------- | ------------------------- | ------ | ------- |
| Logits     | 原始得分，如 `[2.3, 0.7, -1.2]` | ❌      | ✅       |
| Softmax 概率 | `[0.62, 0.25, 0.13]`      | ✅ 和为 1 | ✅       |
| One-hot 标签 | `[1, 0, 0]`               | ✅      | ✅       |
...
> **Logits 是网络在 softmax 之前输出的原始分数，用于分类。它更适合做训练损失输入，而 softmax 概率更适合做推理（预测）输出。**
...
很好！你提到的“交叉熵（**Cross Entropy**）”是 **分类任务中最常见的损失函数**之一，用来衡量模型预测的概率分布与真实分布之间的差距。下面我们来详细理解：
...
> **交叉熵**衡量的是两个概率分布之间的差异。
> 在分类中，它衡量的是：**模型预测的概率分布** 与 **真实类别的 one-hot 分布**之间的距离。
...
* 模型输出 logits（如 `[2.3, 0.7, -1.2]`）

* 对其进行 softmax 得到预测概率分布 `p`：

  ```python
  p = softmax(logits)  # 如 [0.70, 0.25, 0.05]
  ```

* 真实标签是类别 `0`，其 one-hot 编码是：
...
$$
\text{CrossEntropy}(p, y) = -\sum_{i} y_i \cdot \log(p_i)
$$

由于真实标签是 one-hot，所以上式只保留 `y_i = 1` 的那一项：

$$
\text{Loss} = -\log(p_{\text{真实类别}})
$$

也就是说：

> 预测越接近真实类别，交叉熵越小；预测越偏离真实类别，交叉熵越大。

---

## 🔢 举个例子：

如果预测概率是 `[0.70, 0.25, 0.05]`，真实标签是第 0 类：

$$
\text{Loss} = -\log(0.70) \approx 0.357
$$

如果预测是 `[0.10, 0.80, 0.10]`，真实标签还是第 0 类：

$$
\text{Loss} = -\log(0.10) \approx 2.302
$$
...
你通常不需要自己 softmax，因为 `CrossEntropyLoss` 会自动处理：
...
> **交叉熵就是让“正确的类别的概率尽量接近 1”，错误的尽量接近 0。**
...
> **One-hot 编码** 本身不是“伪标签（pseudo-label）”，而是一种用于表示 **确定性标签** 的标准方法；
> 当伪标签被使用时，它**也**可以被编码成 one-hot 形式，但它的本质仍然是“标签的来源不同”。
...
| 概念                    | 定义                                                | 典型形式                   |
| --------------------- | ------------------------------------------------- | ---------------------- |
| **One-hot 编码**        | 一种标签的 **表示方式**：只有一个位置是 1，其余是 0，用来表示“唯一的正确类别”。     | `[0, 0, 1, 0]` 表示第 3 类 |
| **伪标签（Pseudo Label）** | 一种标签的 **来源方式**：不是人工标注，而是由模型预测生成的标签，用于无监督 / 半监督训练。 | 可以是类别编号或 one-hot 编码    |
...
```python
logits → softmax → [0.1, 0.7, 0.2]
```

你可以：

* 取最大值对应的类别 `1`，构造伪标签；
* 把这个伪标签表示成 one-hot：`[0, 1, 0]`；

这个时候，这个 one-hot 是 **伪标签的编码方式**，但本质上是：
...
* **one-hot 是标签的表示方法**，可以用于真实标签、也可以用于伪标签；
* **伪标签是标签的来源**，可以是 hard（如 one-hot）或 soft（如 softmax 概率分布）；
* 在很多方法中，例如 BoostAdapter、PseudoLabeling、自训练等，one-hot 经常用来表示伪标签。
